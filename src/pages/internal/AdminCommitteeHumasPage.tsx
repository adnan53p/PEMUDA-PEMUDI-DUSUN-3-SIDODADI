...
export default function AdminCommitteeHumasPage() {
  ...
  return (
    <InternalLayout title="Humas & Warga" subtitle="Kelola akun Humas, penugasan lapangan, wilayah, permission, dan daftar warga per kegiatan.">
      <InternalNotice />
      <PrototypeToast message={toast} />

      <PrototypeModal open={modalMode === 'create-account'} onClose={() => setModalMode(null)} title="Buat akun Humas" description="Akun Humas dibuat di Supabase Auth oleh Admin. Role dan status aktif diverifikasi server-side.">
        <form onSubmit={createAccount} className="space-y-5">
          ...
          <fieldset><legend className="text-xs font-extrabold uppercase tracking-[0.08em] text-muted">Permission</legend><div className="mt-3 grid gap-2 sm:grid-cols-3">{permissionOptions.map((item) => <label key={item.value} className={`flex cursor-pointer items-center gap-2 border px-3 py-3 text-sm font-semibold ${permissions.includes(item.value) ? 'border-forest bg-sage/45 text-forest' : 'border-border-soft bg-white text-muted'}`}><input type="checkbox" checked={permissions.includes(item.value)} onChange={() => togglePermission(item.value, 'create')} className="accent-[#123D32]" />{item.label}</label>)}</div></fieldset>
      ...
      <PrototypeModal open={modalMode === 'reset-password'} onClose={() => setModalMode(null)} title="Reset password Humas" description="Password akun Supabase diubah server-side. Histori tetap terhubung ke user ID yang sama.">
      ...
      <div className="mt-6"><AdminPageIntro eyebrow="HUMAS & WARGA" title="Fokus pada petugas lapangan dan warga yang menjadi tanggung jawabnya." description="Panitia kegiatan sekarang dikelola langsung dari menu Kegiatan. Di sini Admin hanya mengelola akun Humas, penugasan, wilayah, permission, dan daftar warga iuran." actions={<div className="flex flex-wrap gap-2"><button type="button" onClick={() => setModalMode('add-assignment')} className="btn btn-secondary"><UserRoundCog size={16} /> Tambah Penugasan</button><button type="button" onClick={() => setModalMode('create-account')} className="btn btn-primary"><Plus size={16} /> Buat Akun Humas</button></div>} /></div>
